...
class DirectoryIterator(object):
    ...
    def __init__(self, directory, part="train", for_testing=False, test_temporal_t=1):
        ...
        print(
            "Found {} images belonging to {} experiments.".format(
    ...
    def _parse_data_dir(self, data_dir):
        ...
        for folder_name in categories:
            all_fnames_list_fname = os.path.join(
                data_dir, folder_name, folder_name + ".bmf"
            )
            if not os.path.isfile(all_fnames_list_fname):
                raise IOError("Not found file {}".format(all_fnames_list_fname))
            all_fnames_list = np.loadtxt(
                all_fnames_list_fname, dtype=np.str, skiprows=1
            )
            # Correct from pgm to jpg
            all_fnames_list = [f.split(".")[0] + ".jpg" for f in all_fnames_list]

            all_fnames_list = [
                os.path.join(data_dir, folder_name, f) for f in all_fnames_list
            ]
    ...
    def _parse_testtime_dir(self, data_dir, test_temporal_t=1):
        ...
        for folder_name in categories:
            ...
            # Get ground_truth
            annotation_fnames, numbers, type_weird = self.find_gt(
                os.path.join(data_dir, folder_name, "GroundTruth")
            )
            goal_annotation_fnames = [
                f.split(".")[0] + ".jpg" for f in annotation_fnames
            ]
            goal_annotation_fnames = [
                os.path.join(data_dir, folder_name, "GroundTruth", f)
                for f in goal_annotation_fnames
            ]

            # NOTE: Run the commented part only once to preprocess GT
            annotation_fnames = [
                os.path.join(data_dir, folder_name, "GroundTruth", f)
                for f in annotation_fnames
            ]
            ...
            for i, k in enumerate(numbers):
                self.test_tuples.append(
                    (
                        all_fnames_list[k],
                        all_fnames_list[offsets[i]],
                        goal_annotation_fnames[i],
                        "{}".format(len(annotation_fnames)),
                    )
                )
    ...
    def find_gt(self, directory):
        ...
        if not type_weird:
            ...
            try:
                all_files = sorted(
                    all_files, key=lambda x: int(x.split(".")[0].split("_")[-1])
                )
        ...
        # Solve weird type
        all_files = [
            file for file in all_files if file.endswith("ppm") and not "PROB" in file
        ]
...
class FBMS59Reader(object):
    def __init__(self, root_dir, max_temporal_len=3, min_temporal_len=2, num_threads=6):
    ...
    def get_filenames_list(self, partition):
        ...
        filenames, annotation_filenames = (
    ...
    def get_test_tuples(self, partition, test_temporal_t=1):
        iterator = DirectoryIterator(
            self.root_dir, partition, for_testing=True, test_temporal_t=test_temporal_t
        )
    ...
    def random_crop_image_pair(self, image_1, image_2, max_cropping_percent=0.9):
        ...
        image_c = tf.random_crop(
            image_c, size=[crop_height, crop_width, num_channels * 2]
        )
        image_c.set_shape([None, None, num_channels * 2])
        # Resize
        image_c = tf.image.resize_images(image_c, [image_height, image_width])
        image_1 = image_c[:, :, :3]
        image_2 = image_c[:, :, 3:6]
    ...
    def augment_pair(self, image_1, image_2):
        ...
        image_1, image_2 = self.random_crop_image_pair(
            image_1, image_2, self.train_crop
        )
    ...
    def dataset_map(self, input_queue):
        ...
        t_shift = tf.random_uniform(
            shape=[],
            minval=self.min_temporal_len,
    ...
    def image_inputs(self, batch_size=32, partition="train", train_crop=1.0):
        ...
        for fnames in file_list:
            last_fname_numbers.append(
                np.arange(N + t_len, N + len(fnames), dtype=np.int32)
            )
            first_fname_numbers.append(
                np.arange(N, N + len(fnames) - t_len, dtype=np.int32)
            )
            N += len(fnames)
        ...
        last_fname_numbers = np.vstack(
            (last_fname_numbers, -1.0 * np.ones_like(last_fname_numbers))
        ).T

        first_fname_numbers = np.concatenate(first_fname_numbers)
        first_fname_numbers = np.vstack(
            (first_fname_numbers, 1.0 * np.ones_like(first_fname_numbers))
        ).T
        ...
        dataset = dataset.shuffle(
            buffer_size=all_fname_numbers.shape[0], reshuffle_each_iteration=True
        )
        dataset = dataset.repeat(None)
        dataset = dataset.map(self.dataset_map, num_parallel_calls=self.num_threads)
    ...
    def test_inputs(
        self, batch_size=32, partition="val", t_len=2, with_fname=False, test_crop=1.0
    ):
        ...
        dataset = dataset.repeat(None)
        dataset = dataset.map(self.test_dataset_map, num_parallel_calls=1)
    ...
    def test_dataset_map(self, input_queue):
        fname_1, fname_2, annotation_fname, samples_per_cat = (
            input_queue[0],
            input_queue[1],
            input_queue[2],
            input_queue[3],
    ...
    def augmented_inputs(self, partition="val", t_len=2, test_crops=[1.0]):
        (img_1, img_2, seg_1, fname, _), itr = self.test_inputs(
            batch_size=1,
            t_len=t_len,
            partition=partition,
            with_fname=True,
