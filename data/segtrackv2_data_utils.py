...
class DirectoryIterator(object):
    ...
    def _parse_experiment(self, experiment):
        ...
        current_filenames = []
        current_annotations = []
        experiment_file = os.path.join(self.directory, "ImageSets", experiment + ".txt")
        assert os.path.isfile(experiment_file), "Experiment {} not found".format(
            experiment_file
        )
        ...
        for exp_fname in all_exp_fnames:
            current_filenames.append(
                os.path.join(self.image_dirs, experiment, exp_fname + ".png")
            )
            assert os.path.isfile(current_filenames[-1]), "Not found image {}".format(
                current_filenames[-1]
            )
            current_annotations.append(
                os.path.join(self.annotation_dir, experiment, exp_fname + ".png")
            )
            assert os.path.isfile(current_annotations[-1]), "Not found image {}".format(
...
class SegTrackV2Reader(object):
    ...
    def image_inputs(self, batch_size=32, train_crop=1.0, num_threads=6):
    ...
    def test_inputs(self, batch_size=32, t_len=2, with_fname=False, test_crop=1.0):
        ...
        for fnames in file_list:
            if t_len < 0:
                last_fname_numbers.append(
                    np.arange(N + abs(t_len), N + len(fnames), dtype=np.int32)
                )
                first_fname_numbers.append(np.arange(N, N + abs(t_len), dtype=np.int32))
            elif t_len > 0:
                first_fname_numbers.append(
                    np.arange(N, N + len(fnames) - t_len, dtype=np.int32)
                )
                last_fname_numbers.append(
                    np.arange(N + len(fnames) - t_len, N + len(fnames), dtype=np.int32)
                )
            N += len(fnames)
        ...
        dataset = dataset.repeat(None)
        dataset = dataset.map(
            self.test_dataset_map, num_parallel_calls=self.num_threads
        )
    ...
    def augmented_inputs(self, t_len=2, test_crops=[1.0]):
        ...
        (img_1, img_2, seg_1, fname), itr = self.test_inputs(
            batch_size=1, t_len=t_len, with_fname=True, test_crop=1.0
        )
