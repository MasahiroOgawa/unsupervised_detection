...
class AdversarialLearner(object):
    ...
    def load_training_data(self):
        with tf.name_scope("data_loading"):
            if self.config.dataset == "DAVIS2016":
                ...
                train_batch, train_iter = reader.image_inputs(
                    batch_size=self.config.batch_size, train_crop=self.config.train_crop
                )
    ...
    def build_train_graph(self):
        ...
        gt_masks = tf.image.resize_images(
            val_batch[2],
        ...
        with tf.name_scope("FlownetS") as scope:
            ...
            pred_flows = recover_net(
                image_batch, flow_masked, mask=generated_masks, scope=scope, reuse=False
            )

            pred_complementary_flows = recover_net(
                image_batch,
            ...
            pred_flow_from_image = recover_net(
                image_batch,
        ...
        all_IoU = compute_all_IoU(pred_masks=generated_masks, gt_masks=gt_masks)
        ...
        # Complementary
        rec_compl_loss = charbonnier_loss(
            pred_flows=pred_complementary_flows,
            gt_flows=flow_batch,
        ...
        image_prior_decoder = charbonnier_loss(
            gt_flows=flow_batch,
        ...
        num_pixels = tf.constant(
            self.config.img_width * self.config.img_height * self.config.batch_size,
            dtype=tf.float32,
        )

        recover_loss = (
            recover_red_rate + recover_red_rate_compl + image_prior_decoder
        ) / num_pixels
        ...
        with tf.name_scope("train_op"):
            ...
            recover_vars = tf.get_collection(
                tf.GraphKeys.TRAINABLE_VARIABLES, "FlownetS"
            )
            generator_vars = tf.get_collection(
                tf.GraphKeys.TRAINABLE_VARIABLES, "MaskNet"
            )

            optimizer = tf.train.AdamOptimizer(
                learning_rate=1e-4, beta1=self.config.beta1, epsilon=1e-8
            )
            ...
            self.train_generator_op, self.generator_var_grads = train_op(
            ...
            self.train_recover_op, self.recover_var_grads = train_op(
                loss=losses["recover"],
        ...
        self.pred_flow = pred_flows * generated_masks + flow_batch * (
            1 - generated_masks
        )
        self.pred_flow_compl = pred_flows * complementary_masks + flow_batch * (
            1 - complementary_masks
        )
        self.is_training = is_training_ph
        self.train_steps_per_epoch = int(
            math.ceil(self.config.num_samples_train / self.config.batch_size)
        )
        self.val_steps_per_epoch = int(
            np.ceil(float(self.num_samples_val) / self.config.batch_size)
        )
    ...
    def collect_summaries(self):
        ...
        tf.summary.image(
            "input_image", self.image_batch, max_outputs=1, collections=["step_sum"]
        )
        tf.summary.image(
            "next_image", self.image_2_batch, max_outputs=1, collections=["step_sum"]
        )
        tf.summary.image(
            "masked_flow",
        ...
        for grad, var in self.recover_var_grads:
            tf.summary.histogram(
                var.op.name + "/gradients", grad, collections=["step_sum"]
            )
        ...
        tf.summary.scalar(
            "IoU on Validation", self.val_iou_ph, collections=["validation_summary"]
        )
    ...
    def train(self, config):
        ...
        self.saver = tf.train.Saver(
            [var for var in tf.trainable_variables()] + [self.global_step],
            max_to_keep=40,
        )
        ...
        sv = tf.train.Supervisor(
            logdir=config.checkpoint_dir, save_summaries_secs=0, saver=None
        )
        ...
        with sv.managed_session() as sess:
            ...
            for step in count(start=1):
                ...
                if step % config.summary_freq == 0:
                    ...
                    print(
                        "Epoch: [%2d] [%5d/%5d] time: %4.4f/it loss_generator: %4.4f loss_recover %4.4f"
                        % (
                            train_epoch,
                            train_step,
    ...
    def epoch_end_callback(self, sess, sv, epoch_num):
        ...
        for i in range(self.val_steps_per_epoch):
            loss_iou = sess.run(self.val_iou, feed_dict={self.is_training: False})
            validation_iou += loss_iou
            progbar.update(i)
        validation_iou /= self.val_steps_per_epoch * self.config.batch_size

        # Log to Tensorflow board
        val_sum = sess.run(self.val_sum, feed_dict={self.val_iou_ph: validation_iou})

        sv.summary_writer.add_summary(val_sum, epoch_num)

        print("Epoch [{}] Validation IoU: {}".format(epoch_num, validation_iou))
        # Model Saving
    ...
    def build_test_graph(self):
        ...
        with tf.name_scope("data_loading"):
            ...
            image_batch, images_2_batch, gt_mask_batch, fname_batch = (
                test_batch[0],
                test_batch[1],
                test_batch[2],
                test_batch[3],
    ...
    def build_aug_test_graph(self):
        ...
        with tf.name_scope("data_loading"):
            if self.config.dataset == "DAVIS2016":
                ...
                test_batch, fname_batch, test_iter = reader.augmented_inputs(
                    t_len=self.config.test_temporal_shift, test_crops=test_crops
                )
            else:
                raise IOError("Dataset should be DAVIS2016 / FBMS / SEGTRACK")
    ...
    def inference(self, sess):
        ...
        if self.aug_test:
            ...
            fetches = {
                "gen_masks": self.generated_masks,
                "pred_flow": self.pred_flow,
                "input_image": self.input_image,
                "gt_flow": self.gt_flow,
                "gt_masks": self.gt_masks,
                "img_fname": self.fname_batch,
            }
