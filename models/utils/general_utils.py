...
def tf_iou_computation(gt_masks, pred_masks):
    ...
    union = (
        tf.reduce_sum(
            tf.cast(tf.logical_or(gt_masks, pred_masks), dtype=tf.float32),
            axis=[1, 2, 3],
        )
        + epsilon
    )
    IoU = (
        tf.reduce_sum(
            tf.cast(tf.logical_and(gt_masks, pred_masks), dtype=tf.float32),
            axis=[1, 2, 3],
        )
        / union
    )
...
def compute_boundary_score(segmentation):
    ...
    border_occ = (
        np.sum(up_bord) + np.sum(bottom_bord) + np.sum(left_bord) + np.sum(right_bord)
    )
    border_occ /= 1.0 * (
        up_bord.size + bottom_bord.size + left_bord.size + right_bord.size
    )
    return border_occ
