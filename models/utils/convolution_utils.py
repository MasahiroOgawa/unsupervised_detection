...
def gen_deconv(x, cnum, name="upsample", padding="SAME", training=True):
    ...
    with tf.variable_scope(name):
        ...
        x = gen_conv(
            x, cnum, 3, 1, name=name + "_conv", padding=padding, training=training
        )
    return x
...
def conv(
    ...
    with tf.variable_scope(name, reuse=reuse) as scope:
        kernel = tf.contrib.framework.model_variable(
            "weights", shape=shape, initializer=init_w, trainable=training
        )
        conv = tf.nn.conv2d(inputs, kernel, [1, stride, stride, 1], padding="SAME")
        biases = tf.contrib.framework.model_variable(
            "biases", shape=[shape[3]], initializer=init_b, trainable=training
        )
...
def deconv(
    inputs, size, name, shape, reuse=None, training=True, activation=tf.nn.leaky_relu
):
