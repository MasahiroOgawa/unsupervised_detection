...
def generator_net(images, flows, scope, reuse=None, training=True):
    ...
    with tf.variable_scope(scope, reuse=reuse):
        # stage1
        x_0 = gen_conv(
            x, cnum, 5, 1, name="conv1", training=training
        )  # ---------------------------
        x = gen_conv(
            x_0, 2 * cnum, 3, 2, name="conv2_downsample", training=training
        )  # Skip connection
        x_1 = gen_conv(
            x, 2 * cnum, 3, 1, name="conv3", training=training
        )  # -------------------
        x = gen_conv(x_1, 4 * cnum, 3, 2, name="conv4_downsample", training=training)
        x = gen_conv(x, 4 * cnum, 3, 1, name="conv5", training=training)
        x_2 = gen_conv(
            x, 4 * cnum, 3, 1, name="conv6", training=training
        )  # -----------------
        ...
        x = (
            gen_conv(x, 4 * cnum, 3, 1, name="conv11", training=training) + x_2
        )  # -------------
        x = gen_conv(x, 4 * cnum, 3, 1, name="conv12", training=training)
        x = gen_deconv(x, 2 * cnum, name="conv13_upsample", training=training)
        x = (
            gen_conv(x, 2 * cnum, 3, 1, name="conv14", training=training) + x_1
        )  # --------------------
        x = (
            gen_deconv(x, cnum, name="conv15_upsample", training=training) + x_0
        )  # -------------------
...
def recover_net(img1, flow_masked, mask, scope, reuse=None, f=0.25, training=True):
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        bconv1 = conv(
            flow_masked,
            "bconv1",
            shape=[7, 7, flow_in_channels, int(64 * f)],
            stride=2,
            reuse=reuse,
            training=training,
        )  # h/2(192), 64
        ...
        deconv5 = deconv(
            conv6,
            size=[outsz[1], outsz[2]],
            name="deconv5",
            shape=[4, 4, int(512 * 2 * f), int(512 * f)],
            reuse=reuse,
            training=training,
        )
        ...
        deconv4 = deconv(
            ...
            size=[outsz[1], outsz[2]],
            name="deconv4",
            shape=[4, 4, int(512 * 3 * f), int(512 * f)],
            reuse=reuse,
            training=training,
        )
        upflow4 = deconv(
        ...
        concat4 = tf.concat(
            (deconv4, bconv41, aconv41, upflow4), 3
        )  # h/16(24), 512*3*f+C

        flow4 = conv(
            concat4,
            "flow4",
            shape=[3, 3, int(512 * 3 * f + C), C],
            stride=1,
            reuse=reuse,
            training=training,
            activation=tf.identity,
        ...
        deconv3 = deconv(
            ...
            size=[outsz[1], outsz[2]],
            name="deconv3",
            shape=[4, 4, int(512 * 3 * f + C), int(256 * f)],
            reuse=reuse,
            training=training,
        )
        upflow3 = deconv(
        ...
        concat3 = tf.concat(
            (deconv3, bconv31, aconv31, upflow3), 3
        )  # h/8(48),  256*3*f+C

        flow3 = conv(
            concat3,
            "flow3",
            shape=[3, 3, int(256 * 3 * f + C), C],
            stride=1,
            reuse=reuse,
            training=training,
            activation=tf.identity,
        ...
        deconv2 = deconv(
            ...
            size=[outsz[1], outsz[2]],
            name="deconv2",
            shape=[4, 4, int(256 * 3 * f + C), int(128 * f)],
            reuse=reuse,
            training=training,
        )
        upflow2 = deconv(
        ...
        concat2 = tf.concat(
            (deconv2, bconv2, aconv2, upflow2), 3
        )  # h/4(96),  128*3*f+C

        flow2 = conv(
            concat2,
            "flow2",
            shape=[3, 3, int(128 * 3 * f + C), C],
            stride=1,
            reuse=reuse,
            training=training,
            activation=tf.identity,
        ...
        deconv1 = deconv(
            ...
            size=[outsz[1], outsz[2]],
            name="deconv1",
            shape=[4, 4, int(128 * 3 * f + C), int(64 * f)],
            reuse=reuse,
            training=training,
        )
        upflow1 = deconv(
