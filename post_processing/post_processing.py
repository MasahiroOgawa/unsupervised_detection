...
generate_soft_score_from_buffer.buffer_to_soft_score(
    buffer_path=path_buffer, out_path=out_soft_score
)
...
scomp = 5.0
gauss_k = 0.1
iou_resized = crf_refine.run_crf(
    path_soft, sxy, srgb, scomp, gauss_k, out_path=resized_out
)
...
if benchmark:
    ...
    if not os.path.isdir(original_out):
        ...
        print(original_out)
    iou_original = crf_refine.run_crf_original_resolution(
        resized_out, path_img, path_gt, sxy, srgb, scomp, gauss_k, original_out
    )
