...
def run_crf(path_soft, sxy, srgb, scomp, gauss_k, out_path="./post_processed_davis"):
    ...
    for seq in seq_names:
        ...
        for k in range(seq_len):
            ...
            objscore_m = np.sum(np.multiply(pred_mask, gt_mask)) / (
                np.sum(pred_mask) + 1e-8
            )
            ...
            sio.savemat(
                new_name, {"gt_mask": gt_mask, "soft_mask": mask, "mask": mask_new}
            )
...
def run_crf_original_resolution(
    path_soft,
    path_img,
    path_gt,
    sxy,
    ...
    scomp,
    gauss_k,
    out_path="./post_processed_davis_original",
):
    result_path = path_soft
    seq_names = os.listdir(result_path)
    Sum_iou = 0.0
    ...
    for seq in seq_names:
        ...
        for k in range(seq_len):
            ...
            gt_mask = scipy.misc.imread(gtname) / 255.0
            H, W = gt_mask.shape
            h, w = H * 0.9, W * 0.9
            h, w = int(h), int(w)
