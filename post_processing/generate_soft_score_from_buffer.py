    ...
    for i in range(len(seq_names)):
        ...
        for k in range(seq_num[i]):
            ...
            for shift in range(max_shift):
                ...
                r_name_b = os.path.join(
                    result_path,
                    "%s_%d" % (dprefix, -shift),
                    seq_names[i],
                    "result_%d.mat" % (k),
                )
                r_name_f = os.path.join(
                    result_path,
                    "%s_%d" % (dprefix, shift),
                    seq_names[i],
                    "result_%d.mat" % (k),
            ...
            sio.savemat(
                out_name, {"pred_mask": pred_mask, "img1": img1, "gt_mask": gt_mask}
            )
...
def propagate(out_path, seq_names, seq_num):
    ...
    for i in range(len(seq_names)):
        ...
        for k in range(seq_num[i]):
            ...
            I2 = (np.squeeze(r2["img1"])).astype(float) / 255.0
            I1 = (np.squeeze(r1["img1"])).astype(float) / 255.0
            I2 = I2.copy(order="C")
            I1 = I1.copy(order="C")
