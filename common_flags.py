...
gflags.DEFINE_float(
    "train_crop", 0.9, "minimum random cropping percentage of input images"
)
...
gflags.DEFINE_integer(
    "iters_rec",
    1,
    "training iteration of recover per step."
...
gflags.DEFINE_string(
    "root_dir", "/your/path/to/DAVIS_2016", "Folder containig the evaluation dataset"
)
...
gflags.DEFINE_string(
    "recover_ckpt",
    "",
    "Checkpoint of the pre-trained recover."
...
# Log parameters
gflags.DEFINE_integer(
    "summary_freq", 30, "Logging tensorboard summaries every summary_freq iterations"
)
gflags.DEFINE_integer("save_freq", 5, "Save the latest model every save_freq epochs")
gflags.DEFINE_integer(
    "log_level",
    0,
    "0: no log but save the result images, 1: print log, "
    "2: display image, 3: detailed debug image without stopping, 4: with stop",
)

# Testing parameters
gflags.DEFINE_bool(
    "generate_visualization", False, "Whether to save images while computing metrics"
)
gflags.DEFINE_float(
...
gflags.DEFINE_string(
    "foels_resdir",
    "../../../output/davis",
    "Foels result folder as an input of unsupervised detection evaluation.",
)
